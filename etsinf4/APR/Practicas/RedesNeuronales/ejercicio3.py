...
# Para cada algoritmo de optimización: SGD, Adadelta, Adagrad, Adam
for opt in [optim.Adam] :

    # Para cada función de activación: [Relu, Sigmoid, Tanh]
    for f_act in [nn.ReLU(), nn.Tanh()] :

        # Para 1 a 3 posibles capas ocultas
        for cp in [1,3] :
            if str(f_act) == 'ReLU()' and cp == 1: continue
            if str(f_act) == 'Tanh()' and cp == 3: continue
            for neur in [700] :
                capas_ocultas = [(neur, f_act) for n in range(cp)]
                #print(capas_ocultas)
                mlp = MLP(input_size, capas_ocultas, num_classes, optimizer=opt)
                ...
                if opt == optim.SGD : opt_print = 'SGD'
                if opt == optim.Adadelta : opt_print = 'Adadelta'
                if opt == optim.Adagrad : opt_print = 'Adagrad'
                if opt == optim.Adam : opt_print = 'Adam'

                if str(f_act) == 'ReLU()' : f_act_print = 'ReLU'
                if str(f_act) == 'Sigmoid()' : f_act_print = 'Sigmoid'
                if str(f_act) == 'Tanh()' : f_act_print = 'Tanh'
                
                N = len(test_dataset)
                print('; '.join([opt_print,f_act_print,str(cp),str(neur),str(round(err*100,3)), 
                                 '['+ str(round((err-1.96* math.sqrt((err * (1-err))/N))*100,3))
                                 +','+str(round((err+1.96* math.sqrt((err * (1-err))/N))*100,3))+']']))
