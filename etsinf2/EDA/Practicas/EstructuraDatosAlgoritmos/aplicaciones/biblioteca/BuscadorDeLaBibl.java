...
public class BuscadorDeLaBibl {
    ...
    public ListaConPI<Termino> terminosUnicos() {
        ListaConPI<Termino> res = new LEGListaConPI<Termino>(); //Lista a devolver con terminos que aparecen solo una vez en uno de los libros
        ListaConPI<Termino> claves = this.index.claves(); //Obtenemos las claves
        
        for(claves.inicio(); !claves.esFin(); claves.siguiente()){
            ListaConPI<BuscadorDeLaBibl.Posting> valor = this.index.recuperar(claves.recuperar());
            if(valor.talla()==1) res.insertar(claves.recuperar()); //Si la lista es de tamaño uno significa que solo hay una ocurrencia para dicha clave
        }
