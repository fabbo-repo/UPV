...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    *  Entradas y factor de carga 0.75 */
    ...
    protected final void rehashing() {
        //Guardamas el array actual en una variable auxiliar
        ListaConPI<EntradaHash<C, V>>[] antiguoArray = this.elArray;

        //Procedemos a crear el nuevo array
        int capacidad = this.elArray.length * 2;
        capacidad = siguientePrimo(capacidad);
        this.elArray = new LEGListaConPI[capacidad];
        for (int i = 0; i < elArray.length; i++) {
            elArray[i] = new LEGListaConPI<EntradaHash<C, V>>();}

        //Insertamos los elementos antiguos en el nuevo
        for(int i = 0; i < antiguoArray.length; i++){
            antiguoArray[i].inicio();
            for(int j = 0; j < antiguoArray[i].talla(); j++){
                EntradaHash<C, V> aux = antiguoArray[i].recuperar();
                this.elArray[indiceHash(aux.clave)].insertar(aux);
                antiguoArray[i].siguiente();
            }
        }
    }
    ...
    public final double desviacionTipica() {
        double li = 0.0; double l = 0.0;
        for(int i = 0; i < this.elArray.length; i++){
            l += this.elArray[i].talla();
        }
        l=l/this.elArray.length;
        for(int i = 0; i < this.elArray.length; i++){
            li += Math.pow(elArray[i].talla()-l,2);
        }
        double sigma = Math.sqrt(li/this.elArray.length);
        return sigma;
    }
