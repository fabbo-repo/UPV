...
class SAR_Project:
        ...
        if len(qpartes) <= 2 and not (len(qpartes) == 2 and '"' in qpartes[0]): #Dos opciones: o es un NOT término o es un término
            ...
            if '"' in qpartes[i]: #BUSQUEDA POSICIONAL
                if qpartes[i-1] == "AND" or qpartes[i-1] == "OR" or qpartes[i-1] == "NOT":
                    ...
                    if ':' in posicional[0]:
                        pospartes = posicional[0].split(':')
                        posicional[0] = pospartes[1]
                        res = self.get_posting(" ".join(posicional), pospartes[0])
                    else:
    ...
    def procesarQuery(self, query):
        ...
        for i in tam:
            ...
            if "*" in qterms[i] or "?" in qterms[i]:
                ...
                qterms = qterms + self.sindex['article'][self.stemmer.stem(qterms[i])]
            # Eliminar comillas
            if "\"" in qterms[i]: qterms[i] = qterms[i].replace("\"","")  
        return qterms
    ...
    def rank_result(self, result, query):
        ...
        qpartes = query.split()
        multicampos = []
        for palabra in qpartes:
            if ':' in palabra:
                divpal = palabra.split(':')
                if "date" in divpal[0]:
                    qpartes.remove(palabra)
                else:
                    paldiv = self.tokenize(" ".join(divpal))
                    qpartes.remove(palabra) 
                    multicampos.append(":".join(paldiv))
        if len(qpartes) > 1 or (len(qpartes) == 1 and not ':' in qpartes[0]):
            terminos = self.tokenize(" ".join(qpartes))
        else:
            terminos = query.split()
        terminos += multicampos
        for termino in terminos:
            termino = termino.lower()
            idf = math.log10(totaldocs/len(self.get_posting(termino))) if len(self.get_posting(termino)) > 0 else 0 #idf 
            for newid in result:
                if ':' in termino:
