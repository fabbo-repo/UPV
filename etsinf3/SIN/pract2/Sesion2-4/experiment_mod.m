#!/usr/bin/octave -qf
if (nargin!=4) 
  printf("Usage: ./experiment.m <data> <alphas> <bes> <iteraciones>\n");
  exit(1);
end
...
k=str2num(arg_list{4}); % k = iteraciones
...
NTe = round(.6*N); % 60% es usado para entrenamiento  
...
for a = as
  for b = bs
    [w,E,k] = perceptron(data(1:NTe, :), b, a, k);  
    r1 = zeros(M, 1);
