...
//-----------------------------------------------------
+liderEscogido(LE): name(N) & N==LE // En caso de que el agente actual sea el lider
  <-
  .register_service("lider"); //Para que los fieldops y medics sepan quién es el líder, los soldiers lo saben
  ?flag(F); .goto(F).
//-----------------------------------------------------
+liderEscogido(LE): name(N) // En caso de no ser el lider
  <-
  .giveRing([N], [LE], GR); +ring(GR); // Se obtiene el anillo
  ?flag(F); .givePositionSoldier(GR,F,GPS); // Se obtienen las posiciones para moverse en el anillo
