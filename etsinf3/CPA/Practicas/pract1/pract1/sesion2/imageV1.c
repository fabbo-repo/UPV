           ...
           int n, int m)
{
  ...
  for (p = 0; p < pasos; p++) {    //pasos (no se puede paralelizar)
    for (i = 0; i < n; i++) {       //filas
      for (j = 0; j < m; j++) {    //columnas
        ...
        blue = 0;
        tot = 0;
        for (k = max(0, i - radio); k <= min(n - 1, i + radio); k++) {   //radio por filas

          #pragma omp parallel for private(v) reduction(+:tot) reduction(+:red) reduction(+:green) reduction(+:blue)
          for (l = max(0, j - radio); l <= min(m - 1, j + radio); l++) { //radio por columnas (Se paraleliza en esta version)
            v = ppdBloque[k - i + radio][l - j + radio];    //ponderacion de caadabloque
            red += ppsImagenOrg[k][l].r * v;        //valor de color rojo de dicho pixel 
            green += ppsImagenOrg[k][l].g * v;        //valor de color verde de dicho pixel
            blue += ppsImagenOrg[k][l].b * v;        //valor de color azul de dicho pixel
            tot += v;
          }
...
int main()
{
  ...
  printf("                   \t\tVersion1\tVersion2\tVersion3\tVersion4\n");
  printf("Numero de hilos: %d\t\t%3.3f", iam, t2-t1);
