  ...
  for (p = 0; p < pasos; p++) {    //pasos (no se puede paralelizar)
    for (i = 0; i < n; i++) {       //filas
      for (j = 0; j < m; j++) {    //columnas
        ...
        #pragma omp parallel for private(v,k,l) reduction(+:tot) reduction(+:red) reduction(+:green) reduction(+:blue)
        for (k = max(0, i - radio); k <= min(n - 1, i + radio); k++) {   //radio por filas  (Se paraleliza en esta version)
