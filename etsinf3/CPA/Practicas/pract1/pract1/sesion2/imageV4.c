           ...
           int n, int m)
{
  ...
  for (p = 0; p < pasos; p++) {    //pasos (no se puede paralelizar)
    #pragma omp parallel for private(v,k,l,j) private(tot) private(red) private(green) private(blue)
    for (i = 0; i < n; i++) {       //filas
      for (j = 0; j < m; j++) {    //columnas (Se paraleliza en esta version)
        red = 0;
        green = 0;
        blue = 0;
        tot = 0;
        for (k = max(0, i - radio); k <= min(n - 1, i + radio); k++) {   //radio por filas 
          for (l = max(0, j - radio); l <= min(m - 1, j + radio); l++) { //radio por columnas 
