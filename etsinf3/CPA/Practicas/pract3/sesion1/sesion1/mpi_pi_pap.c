...
int main(int argc,char *argv[])
{
  ...
  /* Sustitución del Reuce por Send y Recv */
  if(myid==0){ 
    pi = mypi;
    for(i=0; i < numprocs-1; i++){ // Hace una recepción de todos los procesos
      MPI_Recv(&mypi, 1, MPI_DOUBLE, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, 
        MPI_STATUS_IGNORE);
      pi+=mypi;
    } 
  } 
