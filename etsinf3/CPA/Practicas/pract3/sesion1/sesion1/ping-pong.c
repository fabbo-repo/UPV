...
/*  Recibe como argumentos n bytes  */
int main(int argc,char *argv[])
{
  ...
  /* COMPLETE: loop of NREPS iterations.
     ...
     the same array. */
   for(i = 0; i <= NREPS-1; i++){
      if(myid == 0) MPI_Send(buf, n, MPI_BYTE, 1, 0, MPI_COMM_WORLD);
      if(myid == 0 || myid == 1)
         MPI_Recv(buf, n, MPI_BYTE, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, 
            MPI_STATUS_IGNORE);
      if(myid == 1) MPI_Send(&buf, n, MPI_BYTE, 0, 0, MPI_COMM_WORLD);
   } 
