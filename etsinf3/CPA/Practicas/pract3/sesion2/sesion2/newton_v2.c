                   ...
                   int me, int np)
{
  ...
  Complejo z0;
  double ix, iy;
  int next_row, num_row, rows_done, proc, flag;
  ...
  if (me == 0) {
    ...
    for ( rows_done = 0 ; rows_done < h ; rows_done++ ) {
      ...
      while(!flag && next_row < h){
        ...
        next_row ++; // siguiente_fila <- siguiente_fila + 1
        rows_done ++; // filas_hechas <- filas_hechas + 1
        MPI_Test(&request, &flag, &status);
      } 
      if(!flag) MPI_Wait(&request, &status); // Espera (de forma bloqueante) a recibir algo
