...
let who=[] // Vector que contiene lista de clientes que esperan ser respondidos
let req=[] // Vector que contiene el mensaje del cliente que espera respuesta
let workers=[]
let tout={} // TablaHash que contiene los TimeOut de los workers que van a responder
...
function resend(c,m) {
	return function() {
		if (workers.length==0) {
			who.push(c); req.push(m) // Añade el cliente y mensaje a la espera
		} else {
